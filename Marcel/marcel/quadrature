...
int main()
{
  std::vector<long double> qpoints = compute_quadrature_points(3,1,1);
  std::vector<long double> qweights = compute_quadrature_weights(qpoints,0,0);
  for (unsigned int i=0;i<qpoints.size();i++) {
      std::cout << qpoints[i] << "    " << qweights[i] << std::endl;
    }
}
